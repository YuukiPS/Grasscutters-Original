...
public class Inventory extends BasePlayerManager implements Iterable<GameItem> {
	...
	public boolean addItem(GameItem item, ActionReason reason, boolean forceNotify) {
		...
		if (item.getItemData().getMaterialType() == MaterialType.MATERIAL_AVATAR) {
			getPlayer()
				.sendPacket(new PacketAddNoGachaAvatarCardNotify((item.getItemId() % 1000) + 10000000, reason, item));
		}
	...
	private void triggerAddItemEvents(GameItem result) {
		getPlayer()
			...
			.triggerMission(WatcherTriggerType.TRIGGER_OBTAIN_MATERIAL_NUM, result.getItemId(), result.getCount());
		getPlayer()
			.getQuestManager()
					...
					default:
						...
						if (existingItem == null) {
							...
							existingItem.setCount(
								Math.min(existingItem.getCount() + item.getCount(), item.getItemData().getStackLimit())
							);
							existingItem.save();
							return existingItem;
						}
	...
	private void addVirtualItem(int itemId, int count) {
		switch (itemId) {
			case 101 -> this.player.getTeamManager() // Character exp
				.getActiveTeam()
				.stream()
				.map(e -> e.getAvatar())
				.forEach(avatar ->
					this.player.getServer().getInventorySystem().upgradeAvatar(this.player, avatar, count)
				);
			case 102 -> this.player.addExpDirectly(count); // Adventure exp
			case 105 -> this.player.getTeamManager() // Companionship exp
				.getActiveTeam()
				.stream()
				.map(e -> e.getAvatar())
				.forEach(avatar ->
					this.player.getServer()
						.getInventorySystem()
						.upgradeAvatarFetterLevel(this.player, avatar, count * (this.player.isInMultiplayer() ? 2 : 1))
				);
			case 106 -> this.player.getResinManager().addResin(count); // Resin
			case 107 -> this.player.addLegendaryKey(count); // Legendary Key
			case 121 -> this.player.getHome().addExp(this.player, count); // Home exp
			case 201 -> this.player.setPrimogems(this.player.getPrimogems() + count); // Primogem
			case 202 -> this.player.setMora(this.player.getMora() + count); // Mora
			case 203 -> this.player.setCrystals(this.player.getCrystals() + count); // Genesis Crystals
			case 204 -> this.player.setHomeCoin(this.player.getHomeCoin() + count); // Home Coin
		}
	}

	private GameItem payVirtualItem(int itemId, int count) {
		switch (itemId) {
			case 201 -> player.setPrimogems(player.getPrimogems() - count); // Primogem
			case 202 -> player.setMora(player.getMora() - count); // Mora
			case 203 -> player.setCrystals(player.getCrystals() - count); // Genesis Crystals
			case 106 -> player.getResinManager().useResin(count); // Resin
			case 107 -> player.useLegendaryKey(count); // LegendaryKey
			case 204 -> player.setHomeCoin(player.getHomeCoin() - count); // Home Coin
			default -> {
	...
	public synchronized boolean payItems(ItemParamData[] costItems, int quantity, ActionReason reason) {
		...
		for (ItemParamData cost : costItems) if (
			this.getVirtualItemCount(cost.getId()) < (cost.getCount() * quantity)
		) return false;
	...
	public synchronized boolean payItems(Iterable<ItemParamData> costItems, int quantity, ActionReason reason) {
		// Make sure player has requisite items
		for (ItemParamData cost : costItems) if (
			getVirtualItemCount(cost.getId()) < (cost.getCount() * quantity)
		) return false;
		// All costs are satisfied, now remove them all
