...
public class InventorySystem extends BaseGameSystem {
	...
	public static synchronized int checkPlayerAvatarConstellationLevel(Player player, int id) {
		...
		if (GameData.getAvatarDataMap().containsKey(id)) {
			...
			avatarId =
				Optional
					.ofNullable(GameData.getItemDataMap().get(id))
	...
	public void upgradeRelic(Player player, long targetGuid, List<Long> foodRelicList, List<ItemParam> list) {
		...
		for (ItemParam itemParam : list) {
			int amount = itemParam.getCount(); // Previously this capped to inventory amount, but rejecting the payment
		...
		while (expGain > 0 && reqExp > 0 && level < relic.getItemData().getMaxLevel()) {
			...
			if (exp >= reqExp) {
				...
				// On relic levelup
				if (
					relic.getItemData().getAddPropLevelSet() != null &&
		...
		// Avatar
		if (oldLevel != level) {
			Avatar avatar = relic.getEquipCharacter() > 0
	...
	public List<ItemParam> calcWeaponUpgradeReturnItems(
		...
		WeaponPromoteData promoteData = GameData.getWeaponPromoteData(
			weapon.getItemData().getWeaponPromoteId(),
			weapon.getPromoteLevel()
		);
		if (promoteData == null) {
			return null;
		}
		...
		// Get exp gain
		int expGain = foodWeaponGuidList
			.stream()
		...
		expGain +=
			itemParamList
				...
				.mapToInt(param -> {
					int exp = Stream
						.ofNullable(GameData.getItemDataMap().get(param.getItemId()))
	...
	public void upgradeWeapon(
		...
		for (ItemParam param : itemParamList) {
			int amount = param.getCount(); // Previously this capped to inventory amount, but rejecting the payment
			// makes more sense for an invalid order
	...
	private List<ItemParam> getLeftoverOres(int leftover) {
		...
		for (var e : weaponRefundMaterials.int2IntEntrySet()) {
			...
			if (ores > 0) leftoverOreList.add(ItemParam.newBuilder().setItemId(e.getIntKey()).setCount(ores).build());
		}
	...
	public void refineWeapon(Player player, long targetGuid, long feedGuid) {
		...
		if (weapon.getItemData().getAwakenMaterial() == 0) {
			...
			if (
				weapon.getItemType() != ItemType.ITEM_WEAPON ||
	...
	public void promoteWeapon(Player player, long targetGuid) {
		...
		WeaponPromoteData currentPromoteData = GameData.getWeaponPromoteData(
			weapon.getItemData().getWeaponPromoteId(),
			weapon.getPromoteLevel()
		);
		WeaponPromoteData nextPromoteData = GameData.getWeaponPromoteData(
			weapon.getItemData().getWeaponPromoteId(),
			nextPromoteLevel
		);
	...
	public void promoteAvatar(Player player, long guid) {
		...
		AvatarPromoteData currentPromoteData = GameData.getAvatarPromoteData(
			avatar.getAvatarData().getAvatarPromoteId(),
			avatar.getPromoteLevel()
		);
		AvatarPromoteData nextPromoteData = GameData.getAvatarPromoteData(
			avatar.getAvatarData().getAvatarPromoteId(),
			nextPromoteLevel
		);
		if (currentPromoteData == null || nextPromoteData == null) {
		...
		// Update proud skills
		Optional
			.ofNullable(GameData.getAvatarSkillDepotDataMap().get(avatar.getSkillDepotId()))
	...
	public void upgradeAvatar(Player player, long guid, int itemId, int count) {
		...
		AvatarPromoteData promoteData = GameData.getAvatarPromoteData(
			avatar.getAvatarData().getAvatarPromoteId(),
			avatar.getPromoteLevel()
		);
		if (promoteData == null) {
		...
		ItemParamData[] costItems = new ItemParamData[] {
			new ItemParamData(itemId, count),
			new ItemParamData(202, moraCost)
		};
			...
			if (data.getDestroyReturnMaterial().length > 0) {
				for (int i = 0; i < data.getDestroyReturnMaterial().length; i++) {
					returnMaterialMap.addTo(
						data.getDestroyReturnMaterial()[i],
						data.getDestroyReturnMaterialCount()[i]
					);
				}
			}
		}
	...
	public synchronized boolean useItemDirect(ItemData itemData, UseItemParams params) {
		...
		if (satiationParams != null && satiationParams.length > 0 && target.isPresent()) {
			...
			if (
				!params.player
					.getSatiationManager()
					.addSatiation(params.targetAvatar, satiationIncrease, itemData.getId())
			) { // Make sure avatar can eat
				return false;
			}
		}
		...
		return actions.stream().map(use -> use.useItem(params)).reduce(false, (a, b) -> a || b); // Don't short-circuit!!!
	}
}
