...
public class WorldDataSystem extends BaseGameSystem {
	...
	public synchronized void loadChestConfig() {
		...
		try {
			DataLoader
				.loadList("ChestReward.json", ChestReward.class)
				.forEach(reward ->
					reward
						.getObjNames()
						.forEach(name ->
	...
	public RewardPreviewData getRewardByBossId(int monsterId) {
		var investigationMonsterData = GameData
	...
	private InvestigationMonsterOuterClass.InvestigationMonster getInvestigationMonster(
		...
		var monster = group.monsters.values().stream().filter(x -> x.monster_id == monsterId).findFirst();
		if (monster.isEmpty()) {
			return null;
		}
