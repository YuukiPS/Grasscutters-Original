...
public class World implements Iterable<Player> {
	...
	@Getter
	private boolean isMultiplayer, timeLocked = false;
	...
	public synchronized void removePlayer(Player player) {
		...
		player.sendPacket(
			new PacketDelTeamEntityNotify(
				...
				this.getPlayers()
					.stream()
					.map(p -> p.getTeamManager().getEntity() == null ? 0 : p.getTeamManager().getEntity().getId())
					.toList()
			)
		);
	...
	public boolean transferPlayerToScene(
		...
		// Get enter types
		val teleportProps = TeleportProperties
			.builder()
			.sceneId(sceneId)
			.teleportType(teleportType)
			.enterReason(enterReason)
	...
	public boolean transferPlayerToScene(Player player, TeleportProperties teleportProperties) {
		...
		if (teleportProperties.getTeleportTo() == null) teleportProperties.setTeleportTo(player.getPosition());
		...
		if (
			teleportProperties.getTeleportType() != TeleportType.INTERNAL &&
		...
		for (Player player : this.getPlayers()) {
			...
			if (this.isMultiplayer()) {
				player
					.getTeamManager()
					.getMpTeam()
					.copyFrom(player.getTeamManager().getMpTeam(), player.getTeamManager().getMaxTeamSize());
	...
	public void lockTime(boolean locked) {
		...
		this.updateTime();
		this.getPlayers().forEach(player -> player.setProperty(PlayerProperty.PROP_IS_GAME_TIME_LOCKED, locked));
	}
