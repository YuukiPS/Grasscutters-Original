...
public final class Scene {
	...
	@Getter
	private final Set<SceneGroup> loadedGroups;
	...
	@Getter
	Int2ObjectMap<Route> sceneRoutes;
	...
	@Getter
	private GameEntity sceneEntity;
	...
	public GameEntity getEntityById(int id) {
		...
		if (id == 0x13800001) return this.sceneEntity; else if (
			id == this.getWorld().getLevelEntityId()
	...
	private void setupPlayerAvatars(Player player) {
		...
		if (
			player.getTeamManager().getCurrentCharacterIndex() >= player.getTeamManager().getActiveTeam().size() ||
	...
	public boolean respawnPlayer(Player player) {
		...
		var teleportProps = TeleportProperties
			.builder()
			.sceneId(getId())
	...
	public void checkSpawns() {
		...
		for (Player player : this.getPlayers()) {
			Collections.addAll(
				loadedGridBlocks,
				SpawnDataEntry.GridBlockId.getAdjacentGridBlockIds(player.getSceneId(), player.getPosition())
			);
		}
	...
	public void checkGroups() {
		...
		for (var group : this.loadedGroups) {
			if (!visible.contains(group.id) && !group.dynamic_load) unloadGroup(
				scriptManager.getBlocks().get(group.block_id),
				group.id
			);
		}
	...
	public void onRegisterGroups() {
		...
		var sceneGroupMap = sceneGroups.stream().collect(Collectors.toMap(item -> item.id, item -> item));
		var sceneGroupsIds = sceneGroups.stream().map(group -> group.id).toList();
		var dynamicGroups = sceneGroups.stream().filter(group -> group.dynamic_load).map(group -> group.id).toList();
	...
	public void loadTriggerFromGroup(SceneGroup group, String triggerName) {
		...
		this.getScriptManager()
			.registerTrigger(group.triggers.values().stream().filter(p -> p.getName().contains(triggerName)).toList());
		group.regions
			.values()
			.stream()
	...
	public void onLoadGroup(List<SceneGroup> groups) {
		...
		for (var group : groups) {
			...
			if (!this.scriptManager.getLoadedGroupSetPerBlock().containsKey(group.block_id)) this.onLoadBlock(
					scriptManager.getBlocks().get(group.block_id),
					players
				);
	...
	private List<SceneNpcBornEntry> loadNpcForPlayer(Player player) {
		...
		var npcList = SceneIndexManager.queryNeighbors(
			data.getIndex(),
			pos.toDoubleArray(),
			Grasscutter.getConfig().server.game.loadEntitiesForPlayerRange
		);

		var sceneNpcBornEntries = npcList.stream().filter(i -> !this.npcBornEntrySet.contains(i)).toList();
