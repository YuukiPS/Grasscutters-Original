...
public final class PlayerProgressManager extends BasePlayerDataManager {
	...
	// `map`.
	public static final Set<Integer> DEFAULT_OPEN_STATES = GameData
		.getOpenStateList()
		.stream()
		.filter(s ->
			s.isDefaultState() &&
			!s.isAllowClientOpen() || // Actual default-opened states.
			(
				(s.getCond().size() == 1) &&
		...
		for (var condition : openState.getCond()) {
			switch (condition.getCondType()) {
				...
				case OPEN_STATE_COND_PARENT_QUEST -> {
					...
					if (mainQuest == null || mainQuest.getState() != ParentQuestState.PARENT_QUEST_STATE_FINISHED) {
						return false;
					}
				}
				// ToDo: Implement.
	...
	public void addReplaceCostumes() {
		...
		GameData
			.getAvatarReplaceCostumeDataMap()
			.keySet()
			.forEach(costumeId -> {
				if (
					GameData.getAvatarCostumeDataMap().get(costumeId) == null ||
