...
public final class TeamManager extends BasePlayerDataManager {
	...
	@Getter
	@Setter
	private boolean usingTrialTeam;

	@Transient
	@Getter
	@Setter
	private TeamInfo trialAvatarTeam;

	// hold trial avatars for later use in rebuilding active team
	@Transient
	@Getter
	@Setter
	private Map<Integer, Avatar> trialAvatars;
	...
	public boolean isSpawned() {
		return (
			this.getPlayer().getScene() != null &&
			this.getPlayer().getScene().getEntities().containsKey(this.getCurrentAvatarEntity().getId())
		);
	}
	...
	public void updateTeamProperties() {
		...
		this.getPlayer().sendPacket(new PacketSceneTeamUpdateNotify(this.getPlayer())); // Notify the player.
	...
	public void trialAvatarTeamPostUpdate(int newCharacterIndex) {
		...
		this.updateTeamProperties();
		if (this.getPlayer().getScene() != null) this.getPlayer().getScene().addEntity(this.getCurrentAvatarEntity());
	}
	...
	public void addAvatarToTrialTeam(Avatar trialAvatar) {
		...
		this.getActiveTeam()
			.forEach(x ->
				this.getPlayer().getScene().removeEntity(x, VisionTypeOuterClass.VisionType.VISION_TYPE_REMOVE)
			);
	...
	public void removeTrialAvatarTeam(List<Integer> trialAvatarIds) {
		...
		this.getActiveTeam()
			.forEach(avatarEntity ->
				scene.removeEntity(avatarEntity, VisionTypeOuterClass.VisionType.VISION_TYPE_REMOVE)
			);
		...
		if (isTeam) {
			...
			trialAvatarIds.forEach(trialAvatarId -> {
	...
	public void onAvatarDie(long dieGuid) {
		...
		if (dieType == PlayerDieType.PLAYER_DIE_TYPE_DRAWN) {
			...
			if (replacement == null) {
				...
				PlayerTeamDeathEvent event = new PlayerTeamDeathEvent(
					this.getPlayer(),
	...
	public void respawnTeam() {
		...
		//		}
		//	}
		this.getPlayer().getStaminaManager().stopSustainedStaminaHandler(); // prevent drowning immediately after respawn
		...
		for (EntityAvatar entity : this.getActiveTeam()) {
			...
			this.getPlayer()
				.sendPacket(new PacketAvatarFightPropUpdateNotify(entity.getAvatar(), FightProperty.FIGHT_PROP_CUR_HP));
			this.getPlayer().sendPacket(new PacketAvatarLifeStateChangeNotify(entity.getAvatar()));
		}
		...
		try {
			...
			this.getPlayer().getPosition().set(GameConstants.START_POSITION); // If something goes wrong, the resurrection is here
		}
	...
	public void applyAbilities(Scene scene) {
		try {
			...
			Grasscutter
				.getLogger()
				.error("Error applying level entity config for scene {}", scene.getSceneData().getId(), e);
		}
	}
	...
	public void removeTrialAvatar() {
		this.removeTrialAvatar(
				this.getActiveTeam().stream().map(EntityAvatar::getAvatar).map(Avatar::getTrialAvatarId).toList()
			);
	}
	...
	public void removeTrialAvatar(List<Integer> trialAvatarIds) {
		...
		this.getPlayer()
			.sendPacket(new PacketAvatarDelNotify(trialAvatarIds.stream().map(this::getTrialAvatarGuid).toList()));
