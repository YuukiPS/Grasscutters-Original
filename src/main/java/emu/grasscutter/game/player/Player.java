...
public class Player implements PlayerHook, FieldFetch {
	...
	@Getter
	private Set<Integer> personalLineList;
	...
	@Getter
	private transient TalkManager talkManager;

	@Getter
	@Setter
	private transient Position lastCheckedPosition = null;
	...
	private final transient Int2ObjectMap<CoopRequest> coopRequests; // Synchronized getter

	@Getter
	private final transient Queue<AttackResult> attackResults;

	@Getter
	private final transient InvokeHandler<CombatInvokeEntry> combatInvokeHandler;

	@Getter
	private final transient InvokeHandler<AbilityInvokeEntry> abilityInvokeHandler;

	@Getter
	private final transient InvokeHandler<AbilityInvokeEntry> clientAbilityInitFinishHandler;
	...
	@Getter
	private transient MpSettingType mpSetting = MpSettingType.MP_SETTING_TYPE_ENTER_AFTER_APPLY;

	@Getter
	private long playerGameTime = 540000; // 9 in-game hours. Present at the start of the game.

	@Getter
	private PlayerProgress playerProgress;

	@Getter
	private Set<Integer> activeQuestTimers;

	@Getter
	@Setter
	private ElementType mainCharacterElement = ElementType.None;
	...
	public boolean setWorldLevel(int level) {
		if (this.setProperty(PlayerProperty.PROP_PLAYER_WORLD_LEVEL, level)) {
			if (
				this.world.getHost() == this
			) this.world.setWorldLevel(level); // Don't update World's WL if we are in someone else's world
			this.updateProfile();
			return true;
		}
		return false;
	...
	private void applyProperties() {
		...
		this.setProperty(PlayerProperty.PROP_CUR_PERSIST_STAMINA, this.getProperty(PlayerProperty.PROP_MAX_STAMINA));
	}
	...
	private void updateWorldLevel() {
		...
		int newWorldLevel = (currentLevel >= 55)
			? 8
			: (currentLevel >= 50)
				? 7
				: (currentLevel >= 45)
					...
					: (currentLevel >= 40)
						...
						: (currentLevel >= 35)
							? 4
							: (currentLevel >= 30) ? 3 : (currentLevel >= 25) ? 2 : (currentLevel >= 20) ? 1 : 0;
	...
	public void onEnterRegion(SceneRegion region) {
		this.getQuestManager()
			.forEachActiveQuest(quest -> {
				if (
					quest.getTriggerData() != null &&
					...
					if (!Boolean.TRUE.equals(quest.getTriggers().put("ENTER_REGION_" + region.config_id, true))) {
						...
						this.getQuestManager()
							.queueEvent(
								...
								quest.getTriggerData().get("ENTER_REGION_" + region.config_id).getId(),
								0
							);
					}
				}
				...
				if (quest.getTriggers().containsKey("LEAVE_REGION_" + region.config_id)) {
					...
					if (!Boolean.TRUE.equals(quest.getTriggers().put("LEAVE_REGION_" + region.config_id, true))) {
						...
						this.getQuestManager()
							.queueEvent(
								QuestContent.QUEST_CONTENT_TRIGGER_FIRE,
								quest.getTriggerData().get("LEAVE_REGION_" + region.config_id).getId(),
								0
							);
					}
				}
	...
	public OnlinePlayerInfo getOnlinePlayerInfo() {
		OnlinePlayerInfo.Builder onlineInfo = OnlinePlayerInfo
			.newBuilder()
			.setUid(this.getUid())
			...
			if (this.getShowAvatarList() != null) {
				for (int avatarId : this.getShowAvatarList()) {
					socialShowAvatarInfoList.add(
						...
						SocialShowAvatarInfoOuterClass.SocialShowAvatarInfo
							.newBuilder()
	...
	public PlayerWorldLocationInfoOuterClass.PlayerWorldLocationInfo getWorldPlayerLocationInfo() {
		return PlayerWorldLocationInfoOuterClass.PlayerWorldLocationInfo
			.newBuilder()
	...
	private synchronized void doDailyReset() {
		...
		var lastResetDate = LocalDate.ofInstant(
			Instant.ofEpochSecond(this.getLastDailyReset()),
