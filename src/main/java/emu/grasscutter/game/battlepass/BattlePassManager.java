...
public class BattlePassManager extends BasePlayerDataManager {
	...
	public void addPointsDirectly(int points, boolean isWeekly) {
		...
		if (
			this.point >= GameConstants.BATTLE_PASS_POINT_PER_LEVEL &&
	...
	public void takeReward(List<BattlePassRewardTakeOption> takeOptionList) {
		...
		for (BattlePassRewardTakeOption option : takeOptionList) {
			...
			BattlePassRewardData rewardData = GameData
		...
		if (rewardList.size() > 0) {
			...
			for (var option : rewardList) {
				...
				BattlePassReward bpReward = new BattlePassReward(
					tag.getLevel(),
					tag.getRewardId(),
	...
	public void resetDailyMissions() {
		...
		for (var mission : this.missions.values()) {
			if (
				mission.getData().getRefreshType() == null ||
				mission.getData().getRefreshType() == BattlePassMissionRefreshType.BATTLE_PASS_MISSION_REFRESH_DAILY
			) {
	...
	public BattlePassSchedule getScheduleProto() {
		...
		var nextSundayTime = LocalDateTime.of(
		...
		BattlePassSchedule.Builder schedule = BattlePassSchedule
			.newBuilder()
			.setScheduleId(2700)
