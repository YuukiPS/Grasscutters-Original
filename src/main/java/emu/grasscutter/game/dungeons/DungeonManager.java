...
public final class DungeonManager {
	...
	public boolean activateRespawnPoint(int pointId) {
		...
		scene.broadcastPacket(
			new PacketDungeonWayPointNotify(activeDungeonWayPoints.add(pointId), activeDungeonWayPoints)
		);
			...
			case DUNGEON_ACTIVITY -> {
				switch (getDungeonData().getPlayType()) {
					case DUNGEON_PLAY_TYPE_TRIAL_AVATAR -> {
						val activityHandler = player
							.getActivityManager()
							.getActivityHandlerAs(
								ActivityType.NEW_ACTIVITY_TRIAL_AVATAR,
								TrialAvatarActivityHandler.class
							);
	...
	public void finishDungeon() {
		...
		this.getScene().getPlayers().forEach(player -> player.getPlayerProgress().markDungeonAsComplete(dungeonId));
	...
	public void notifyEndDungeon(boolean successfully) {
		scene
			...
			.forEach(p -> {
				...
				if (!successfully) {
					p.getQuestManager().queueEvent(QuestContent.QUEST_CONTENT_FAIL_DUNGEON, dungeonData.getId());
				}
