...
public class CombineManger extends BaseGameSystem {
	...
	public static void initialize() {
		...
		try {
			DataLoader
				.loadList("ReliquaryDecompose.json", ReliquaryDecomposeEntry.class)
				.forEach(entry -> {
	...
	public CombineResult combineItem(Player player, int cid, int count) {
		...
		result.setResult(
			List.of(new ItemParamData(combineData.getResultItemId(), combineData.getResultItemCount() * count))
		);
