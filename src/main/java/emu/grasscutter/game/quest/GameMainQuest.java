...
public class GameMainQuest {
	...
	private void addAllChildQuests() {
		List<Integer> subQuestIds = Arrays
			.stream(GameData.getMainQuestDataMap().get(this.parentQuestId).getSubQuests())
		...
		for (var subQuestId : subQuestIds) {
			...
			if (questConfig == null) {
				Grasscutter
					.getLogger()
					.error("Quest {} not found in QuestData. Please check MainQuestData and QuestData.", subQuestId);
				continue;
			}
	...
	public void incQuestVar(int i, int inc) {
		...
		Grasscutter
			.getLogger()
			.debug("questVar {} value incremented from {} to {}", i, previousValue, previousValue + inc);
	...
	public void decQuestVar(int i, int dec) {
		...
		Grasscutter
			.getLogger()
			.debug("questVar {} value decremented from {} to {}", i, previousValue, previousValue - dec);
	...
	public void finish() {
		...
		synchronized (this) {
			if (this.isFinished || this.state == ParentQuestState.PARENT_QUEST_STATE_FINISHED) {
				Grasscutter
					.getLogger()
					.debug("Skip main quest {} finishing because it's already finished", this.getParentQuestId());
				return;
			}
	...
	public List<Position> rewindTo(GameQuest targetQuest, boolean notifyDelete) {
		...
		List<GameQuest> rewindQuests = getChildQuests()
			.values()
			.stream()
			.filter(p ->
				(
					p.getState() == QuestState.QUEST_STATE_UNFINISHED || p.getState() == QuestState.QUEST_STATE_FINISHED
				) &&
				p.getQuestData() != null &&
	...
	public List<Position> rewind() {
		...
		var highestActiveQuest = activeQuests
			.stream()
		...
		if (highestActiveQuest == null) {
			var firstUnstarted = getChildQuests()
				.values()
				.stream()
		...
		var rewindTarget = getChildQuests()
			.values()
			.stream()
			.filter(q -> q.getQuestData() != null)
	...
	public boolean hasRewindPosition(int subId, List<Position> posAndRot) {
		...
		ScriptSceneData.ScriptObject dummyPointScript = fullGlobals
			.getScriptObjectList()
	...
	public boolean hasTeleportPosition(int subId, List<Position> posAndRot) {
		...
		TeleportData.TransmitPoint transmitPoint = questTransmit.getTransmit_points().size() > 0
	...
	public void tryAcceptSubQuests(QuestCond condType, String paramStr, int... params) {
		try {
			List<GameQuest> subQuestsWithCond = getChildQuests()
				.values()
				.stream()
				...
				.filter(p ->
					p
						...
						.getAcceptCond()
						.stream()
						.anyMatch(q -> condType == QuestCond.QUEST_COND_NONE || q.getType() == condType)
				)
				.toList();
			...
			for (GameQuest subQuestWithCond : subQuestsWithCond) {
				...
				for (int i = 0; i < subQuestWithCond.getQuestData().getAcceptCond().size(); i++) {
					...
					boolean result = questSystem.triggerCondition(
						getOwner(),
						subQuestWithCond.getQuestData(),
						condition,
						paramStr,
						params
					);
	...
	public void tryFailSubQuests(QuestContent condType, String paramStr, int... params) {
		try {
			...
			for (GameQuest subQuestWithCond : subQuestsWithCond) {
				...
				boolean shouldFail = LogicType.calculate(
	...
	public void tryFinishSubQuests(QuestContent condType, String paramStr, int... params) {
		try {
			List<GameQuest> subQuestsWithCond = getChildQuests()
				...
				.filter(p ->
					p.getState() == QuestState.QUEST_STATE_UNFINISHED && p.getQuestData().getAcceptCond() != null
				)
			...
			for (GameQuest subQuestWithCond : subQuestsWithCond) {
				...
				boolean shouldFinish = LogicType.calculate(
	...
	public boolean initTimeVar(int index) {
		if (index >= this.timeVar.length) {
			Grasscutter
				.getLogger()
				.error("Trying to init out of bounds time var {} for quest {}", index, this.parentQuestId);
			return false;
		}
	...
	public boolean clearTimeVar(int index) {
		if (index >= this.timeVar.length) {
			Grasscutter
				.getLogger()
				.error("Trying to clear out of bounds time var {} for quest {}", index, this.parentQuestId);
			return false;
		}
	...
	public long getDaysSinceTimeVar(int index) {
		if (index >= this.timeVar.length) {
			Grasscutter
				.getLogger()
				.error("Trying to get days for out of bounds time var {} for quest {}", index, this.parentQuestId);
			return -1;
		}
	...
	public long getHoursSinceTimeVar(int index) {
		if (index >= this.timeVar.length) {
			Grasscutter
				.getLogger()
				.error("Trying to get hours for out of bounds time var {} for quest {}", index, this.parentQuestId);
			return -1;
		}
		val varTime = timeVar[index];
