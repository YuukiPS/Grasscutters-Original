...
public class StaminaManager extends BasePlayerManager {
	...
	public int updateStaminaRelative(GameSession session, Consumption consumption, boolean isCharacterStamina) {
		...
		for (Map.Entry<String, BeforeUpdateStaminaListener> listener : beforeUpdateStaminaListeners.entrySet()) {
			Consumption overriddenConsumption = listener
				.getValue()
				.onBeforeUpdateStamina(consumption.type.toString(), consumption, isCharacterStamina);
			if (
				(overriddenConsumption.type != consumption.type) && (overriddenConsumption.amount != consumption.amount)
			) {
				logger.debug(
					"Stamina update relative(" +
					consumption.type.toString() +
					", " +
					consumption.amount +
	...
	public int updateStaminaAbsolute(GameSession session, String reason, int newStamina, boolean isCharacterStamina) {
		...
		// notify will update
		for (Map.Entry<String, BeforeUpdateStaminaListener> listener : beforeUpdateStaminaListeners.entrySet()) {
			int overriddenNewStamina = listener
				.getValue()
				.onBeforeUpdateStamina(reason, newStamina, isCharacterStamina);
	...
	public void killAvatar(GameSession session, GameEntity entity, PlayerDieType dieType) {
		session.send(
			new PacketAvatarLifeStateChangeNotify(
				player.getTeamManager().getCurrentAvatarEntity().getAvatar(),
				LifeState.LIFE_DEAD,
				dieType
			)
		);
	...
	public void handleCombatInvocationsNotify(
		@NotNull GameSession session,
		@NotNull EntityMoveInfo moveInfo,
	...
	private void handleImmediateStamina(GameSession session, @NotNull MotionState motionState) {
		...
		switch (motionState) {
			case MOTION_STATE_CLIMB -> updateStaminaRelative(
				session,
				new Consumption(ConsumptionType.CLIMB_START),
				true
			);
			case MOTION_STATE_DASH_BEFORE_SHAKE -> updateStaminaRelative(
				session,
				new Consumption(ConsumptionType.SPRINT),
				true
			);
			case MOTION_STATE_CLIMB_JUMP -> updateStaminaRelative(
				session,
				new Consumption(ConsumptionType.CLIMB_JUMP),
				true
			);
			case MOTION_STATE_SWIM_DASH -> updateStaminaRelative(
				session,
				new Consumption(ConsumptionType.SWIM_DASH_START),
				true
			);
			default -> {}
		}
	...
	private class SustainedStaminaHandler extends TimerTask {
		...
		public void run() {
			...
			if (
				moving || (currentCharacterStamina < maxCharacterStamina) || (currentVehicleStamina < maxVehicleStamina)
			) {
				logger.trace(
					"Player moving: " +
					moving +
					", stamina full: " +
					(currentCharacterStamina >= maxCharacterStamina) +
					", recalculate stamina"
				);
