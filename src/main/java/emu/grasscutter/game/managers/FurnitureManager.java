...
public class FurnitureManager extends BasePlayerManager {
	...
	public void startMake(int makeId, int avatarId) {
		...
		if (makeData == null) {
			player
				.getSession()
				.send(new PacketFurnitureMakeStartRsp(Retcode.RET_FURNITURE_MAKE_CONFIG_ERROR_VALUE, null));
			return;
		}

		// check slot count
		if (
			player.getHome().getLevelData().getFurnitureMakeSlotCount() <=
		...
		if (!player.getInventory().payItems(makeData.getMaterialItems())) {
			player
				.getSession()
				.send(new PacketFurnitureMakeStartRsp(Retcode.RET_HOME_FURNITURE_COUNT_NOT_ENOUGH_VALUE, null));
			return;
		}

		var furnitureSlot = FurnitureMakeSlotItem
			.of()
			.avatarId(avatarId)
			.makeId(makeId)
	...
	public void take(int index, int makeId, boolean isFastFinish) {
		...
		if (makeData == null) {
			player
				.getSession()
				.send(
					new PacketTakeFurnitureMakeRsp(Retcode.RET_FURNITURE_MAKE_CONFIG_ERROR_VALUE, makeId, null, null)
				);
			return;
		}
		...
		if (slotItem.isEmpty()) {
			player
				.getSession()
				.send(
					new PacketTakeFurnitureMakeRsp(Retcode.RET_FURNITURE_MAKE_NO_MAKE_DATA_VALUE, makeId, null, null)
				);
			return;
		}
		...
		if (isFastFinish && !player.getInventory().payItem(107013, 1)) {
			player
				.getSession()
				.send(new PacketTakeFurnitureMakeRsp(Retcode.RET_FURNITURE_MAKE_UNFINISH_VALUE, makeId, null, null));
			return;
		}
