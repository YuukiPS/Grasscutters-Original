...
public class CookingCompoundManager extends BasePlayerManager {
	...
	public static void initialize() {
		...
		if (
			compoundGroups.containsKey(3)
		) unlocked.addAll(compoundGroups.get(3)); // Avoid NPE from Resources error
	}
	...
	public synchronized void handlePlayerCompoundMaterialReq(PlayerCompoundMaterialReq req) {
		...
		if (
			activeCompounds.containsKey(id) && activeCompounds.get(id).getTotalCount() + count > compound.getQueueSize()
		) {
			...
			player.sendPacket(
				new PackageTakeCompoundOutputRsp(
					allRewards
						.values()
						.stream()
						.map(i -> ItemParam.newBuilder().setItemId(i.getItemId()).setCount(i.getCount()).build())
						.toList(),
