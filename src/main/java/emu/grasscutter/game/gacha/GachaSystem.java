...
public class GachaSystem extends BaseGameSystem {
	...
	private synchronized int doFallbackRarePull(
		...
		if (fallback1.length < 1) {
			...
			int chosenPool =
				switch ((pityPool1 >= pityPool2) ? 1 : 0) { // Larger weight must come first for the hard cutoff to function correctly
	...
	public synchronized void doPulls(Player player, int scheduleId, int times) {
		...
		if (
			inventory.getInventoryTab(ItemType.ITEM_WEAPON).getSize() +
			times >
		...
		for (int i = 0; i < times; i++) {
			...
			switch (constellation) {
				...
				default:
					if (constellation >= 6) { // C6, give consolation starglitter
						...
						int constItemId = itemId + 100; // This may not hold true for future characters. Examples of strictly
			...
			if (addStarglitter > 0) {
				ItemParam starglitterParam = ItemParam
					.newBuilder()
					.setItemId(starglitterId)
					.setCount(addStarglitter)
					.build();
				if (isTransferItem) {
