		...
		return switch (params.itemUseTarget) {
			...
			case ITEM_USE_TARGET_CUR_TEAM -> {
				...
				for (int i = 0; i < activeTeam.size(); i++) {
					...
					if (i == currentCharacterIndex) this.addEnergy(avatar, params.count); else this.addEnergy(
							avatar,
							params.count * offFieldRatio
						);
				}
			...
			case ITEM_USE_TARGET_SPECIFY_AVATAR,
				...
				ITEM_USE_TARGET_SPECIFY_DEAD_AVATAR -> this.addEnergy(params.targetAvatar, params.count); // Targeted items might care about this
