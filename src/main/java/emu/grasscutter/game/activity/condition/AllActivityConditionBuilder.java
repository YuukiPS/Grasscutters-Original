...
public class AllActivityConditionBuilder {
	...
	private Map<ActivityConditions, ActivityConditionBaseHandler> initActivityConditions() {
		...
		return reflector
			.getTypesAnnotatedWith(ActivityCondition.class)
			.stream()
	...
	private ActivityConditionBaseHandler newInstance(Class<?> clazz) {
		try {
			...
			Grasscutter
				.getLogger()
				.error("Failed to initiate activity condition: {}, object have wrong type", clazz.getName());
		} catch (Exception e) {
			String message = String.format(
				"Failed to initiate activity condition: %s, %s",
				clazz.getName(),
				e.getMessage()
			);
