...
public interface HandbookActions {
	...
	static Response grantAvatar(GrantAvatar request) {
		...
		try {
			...
			Objects
				.requireNonNull(avatar.getSkillDepot())
			...
			return Response.builder().status(500).message("An error occurred while granting the avatar.").build();
		}
	}
	...
	static Response giveItem(GiveItem request) {
		...
		try {
			...
			Grasscutter.getLogger().debug("A handbook command error occurred.", exception);
			return Response.builder().status(500).message("An error occurred while granting the item.").build();
		}
	}
	...
	static Response teleportTo(TeleportTo request) {
		...
		try {
			...
			Grasscutter.getLogger().debug("A handbook command error occurred.", exception);
			return Response.builder().status(500).message("An error occurred while teleporting to the scene.").build();
		}
	}
