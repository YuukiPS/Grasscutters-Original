...
public class EntityAvatar extends GameEntity {
			...
			getScene()
				.broadcastPacket(
					new PacketEntityFightPropChangeReasonNotify(
						...
						healed,
						mute ? PropChangeReason.PROP_CHANGE_REASON_NONE : PropChangeReason.PROP_CHANGE_REASON_ABILITY,
	...
	public void clearEnergy(ChangeEnergyReason reason) {
		...
		if (reason == ChangeEnergyReason.CHANGE_ENERGY_REASON_SKILL_START) {
			this.getScene()
				.broadcastPacket(new PacketEntityFightPropChangeReasonNotify(this, curEnergyProp, -curEnergy, reason));
		}
	}
	...
	public void addEnergy(float amount, PropChangeReason reason, boolean isFlat) {
		...
		if (newEnergy != curEnergy) {
			...
			this.getScene()
				.broadcastPacket(new PacketEntityFightPropChangeReasonNotify(this, curEnergyProp, newEnergy, reason));
		}
	}
	...
	public SceneAvatarInfo getSceneAvatarInfo() {
		...
		SceneAvatarInfo.Builder avatarInfo = SceneAvatarInfo
			.newBuilder()
	...
	public AbilityControlBlock getAbilityControlBlock() {
		...
		if (data.getAbilities() != null) {
			for (int id : data.getAbilities()) {
				AbilityEmbryo emb = AbilityEmbryo
					.newBuilder()
	...
	public void move(Position newPosition, Position rotation) {
		...
		PlayerMoveEvent event = new PlayerMoveEvent(
			this.getPlayer(),
			PlayerMoveEvent.MoveType.PLAYER,
			this.getPosition(),
			newPosition
		);
		event.call();
