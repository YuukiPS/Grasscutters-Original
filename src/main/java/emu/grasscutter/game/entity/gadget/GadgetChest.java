	...
	public boolean onInteract(Player player, GadgetInteractReq req) {
		...
		if (Grasscutter.getConfig().server.game.enableScriptInBigWorld) {
			...
			if (chest.boss_chest != null && chest.drop_tag != null) {
				...
				if (
					player.getResinManager().useResin(chest.boss_chest.resin) &&
				...
				if (status) {
					...
					player.sendPacket(
						new PacketWorldChestOpenNotify(getGadget().getGroupId(), player.getSceneId(), chest.config_id)
					);
					return true;
				}
				// if failed,fallback to legacy drop system.
		...
		var chestInteractHandlerMap = getGadget()
			.getScene()
			.getWorld()
			.getServer()
			.getWorldDataSystem()
