...
public class Avatar {
	...
	@Transient
	private Map<Integer, Integer> proudSkillBonusMap = new Int2IntArrayMap(2); // Talent bonus levels (from const)
	...
	public boolean isMainCharacter() {
		return List
			.of(GameConstants.MAIN_CHARACTER_MALE, GameConstants.MAIN_CHARACTER_FEMALE)
	...
	public Map<Integer, Integer> getSkillLevelMap() { // Returns a copy of the skill levels for the current skillDepot.
		...
		this.skillDepot.getSkillsAndEnergySkill()
			.forEach(skillId -> map.put(skillId, this.skillLevelMap.putIfAbsent(skillId, 1).intValue()));
		return map;
	}
	...
	public boolean equipItem(GameItem item, boolean shouldRecalc) {
		...
		if (otherAvatar != null) {
			...
			if (otherAvatar.unequipItem(item.getItemData().getEquipType())) {
				getPlayer()
					.sendPacket(new PacketAvatarEquipChangeNotify(otherAvatar, item.getItemData().getEquipType()));
			}
	...
	public void recalcStats(boolean forceSendAbilityChange) {
		...
		float currentEnergy = (this.getSkillDepot() != null)
		...
		for (int slotId = 1; slotId <= 5; slotId++) {
			...
			if (mainPropData != null) {
				ReliquaryLevelData levelData = GameData.getRelicLevelData(
					equip.getItemData().getRankLevel(),
					equip.getLevel()
				);
				if (levelData != null) {
					this.addFightProperty(
							mainPropData.getFightProp(),
		...
		if (getPlayer() != null && getPlayer().hasSentLoginPackets()) {
			...
			if (
				entity != null &&
				(!this.getExtraAbilityEmbryos().equals(prevExtraAbilityEmbryos) || forceSendAbilityChange)
			) {
	...
	public void calcConstellation(OpenConfigEntry entry, boolean notifyClient) {
		...
		if (this.calcConstellationExtraCharges(entry) && notifyClient) {
			// Packet
			Stream
				.of(entry.getSkillPointModifiers())
	...
	public AvatarInfo toProto() {
		...
		if (this.fetters != null) {
			this.fetters.forEach(fetterId ->
					avatarFetter.addFetterList(
						FetterData.newBuilder().setFetterId(fetterId).setFetterState(FetterState.FINISH.getValue())
					)
				);
		}
		...
		AvatarInfo.Builder avatarInfo = AvatarInfo
			.newBuilder()
		...
		avatarInfo.putPropMap(
			...
			ProtoHelper.newPropValue(PlayerProperty.PROP_SATIATION_PENALTY_TIME, this.getSatiationPenalty())
		);
	...
	public ShowAvatarInfo toShowAvatarInfoProto() {
		...
		ShowAvatarInfo.Builder showAvatarInfo = ShowAvatarInfoOuterClass.ShowAvatarInfo
			.newBuilder()
		...
		for (GameItem item : this.getEquips().values()) {
			if (item.getItemType() == ItemType.ITEM_RELIQUARY) {
				showAvatarInfo.addEquipList(
					ShowEquip.newBuilder().setItemId(item.getItemId()).setReliquary(item.toReliquaryProto())
				);
	...
	public int getTrialWeaponId() {
		// Use default data if custom data not available.
		if (GameData.getTrialAvatarCustomData().isEmpty()) {
			if (GameData.getTrialAvatarDataMap().get(this.getTrialAvatarId()) == null) return this.getAvatarData()
				.getInitialWeapon();
	...
	public List<Integer> getTrialReliquary() {
		...
		if (GameData.getTrialAvatarCustomData().isEmpty()) {
			...
			TrialAvatarTemplateData templateData = GameData
				.getTrialAvatarTemplateDataMap()
				.get(trialAvatarTemplateLevel);
	...
	public TrialAvatarInfo toTrialInfo() {
		var trialAvatar = TrialAvatarInfo
			.newBuilder()
