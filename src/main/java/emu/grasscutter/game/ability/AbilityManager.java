...
public final class AbilityManager extends BasePlayerManager {
	...
	private static final HashMap<AbilityModifierAction.Type, AbilityActionHandler> actionHandlers = new HashMap<>();
	private static final HashMap<AbilityMixinData.Type, AbilityMixinHandler> mixinHandlers = new HashMap<>();
	...
	private void handleModifierChange(AbilityInvokeEntry invoke) throws Exception {
		...
		if (modChange.getAction() == ModifierAction.MODIFIER_ACTION_ADDED) {
			...
			AbilityModifierController modifier = new AbilityModifierController(
				instancedAbility,
				instancedAbilityData,
				modifierData
			);
	...
	private void handleGlobalFloatValue(AbilityInvokeEntry invoke) throws InvalidProtocolBufferException {
		...
		String key = null;
		if (entry.getKey().hasStr()) key = entry.getKey().getStr(); else if (entry.getKey().hasHash()) key =
			GameData.getAbilityHashes().get(entry.getKey().getHash());
		...
		switch (entry.getValueType().getNumber()) {
			case AbilityScalarType.ABILITY_SCALAR_TYPE_FLOAT_VALUE:
				if (!Float.isNaN(entry.getFloatValue())) entity
					.getGlobalAbilityValues()
					.put(key, entry.getFloatValue());
				break;
	...
	private void handleAddNewAbility(AbilityInvokeEntry invoke) throws InvalidProtocolBufferException {
		...
		Grasscutter
			.getLogger()
			.trace("Ability added to entity {} at index {}", entity.getId(), entity.getInstancedAbilities().size());
	}
