...
public interface DispatchUtils {
	...
	static Account authenticate(String accountId, String token) {
		return switch (Grasscutter.getRunMode()) {
			case GAME_ONLY -> // Use the authentication system to validate the token.
			Grasscutter
	 ...
	 */
	@Nullable
	static String fetchSessionKey(int playerId) {
		return switch (Grasscutter.getRunMode()) {
			case GAME_ONLY -> {
	 ...
	 */
	@Nullable
	static Account getAccountById(String accountId) {
		return switch (Grasscutter.getRunMode()) {
			case GAME_ONLY -> {
	 ...
	 */
	@Nullable
	static JsonObject getPlayerFields(int playerId, String... fields) {
		return switch (Grasscutter.getRunMode()) {
			case DISPATCH_ONLY -> {
	 ...
	 */
	@Nullable
	static JsonObject getPlayerByAccount(String accountId, String... fields) {
		return switch (Grasscutter.getRunMode()) {
			case DISPATCH_ONLY -> {
				// Create a request for player fields.
		...
		return switch (Grasscutter.getRunMode()) {
			case DISPATCH_ONLY -> {
				...
				try {
					...
					yield Response.builder().status(400).message("No response received from any server.").build();
				}
			}
