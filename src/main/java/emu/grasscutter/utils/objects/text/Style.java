...
public final class Style {
	...
	@Builder.Default
	private int size = -1; // Unity only.

	@Builder.Default
	private boolean bold = false; // Unity only.

	@Builder.Default
	private boolean italic = false; // Unity only.
	...
	public String toUnity(String text) {
		...
		if (this.color != null) {
			builder
				.append("<color=")
				.append(
					String.format("#%02x%02x%02x", this.color.getRed(), this.color.getGreen(), this.color.getBlue())
				)
				.append(">");
		}
	...
	public String toTerminal(String text) {
		...
		var ansiColor = this.color.getRed() > 127
			? this.color.getGreen() > 127 ? this.color.getBlue() > 127 ? 15 : 11 : this.color.getBlue() > 127 ? 13 : 9
			: this.color.getGreen() > 127 ? this.color.getBlue() > 127 ? 14 : 10 : this.color.getBlue() > 127 ? 12 : 8;
