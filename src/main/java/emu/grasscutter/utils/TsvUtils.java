...
public final class TsvUtils {
	...
	private static final Set<Type> primitiveTypes = Set.of(
		String.class,
		Integer.class,
		int.class,
		Long.class,
	...
	private static <T> Pair<Constructor<T>, String[]> getAllArgsConstructor(Class<T> classType) {
		for (var c : classType.getDeclaredConstructors()) {
			val consParameters = (java.beans.ConstructorProperties) c.getAnnotation(
	...
	public static <T> List<List<T>> loadTsjsToListsConstructor(Class<T> classType, Path... filenames) throws Exception {
		...
		val argTypes = new Type[numArgs]; // constructor.getParameterTypes() returns base types like java.util.List
						...
						.map(line -> {
							...
							try {
								...
								Grasscutter
									...
									.warn(
										"Header types are: " +
										IntStream.of(argPositions).mapToObj(i -> (i >= 0) ? argTypes[i] : null).toList()
									);
								Grasscutter.getLogger().warn("Tokens are: " + tokens);
								Grasscutter.getLogger().warn("Stacktrace is: ", e);
								return null;
	...
	private static class StringValueTree {
		...
		private <K, V> Map<K, V> toMap(Class<K> keyClass, Class<V> valueClass, Type valueType) {
			...
			this.children.forEach((key, tree) -> {
					if ((key != null) && !key.isEmpty()) map.put(
						(K) keyParser.apply(key),
						tree.toClass(valueClass, valueType)
					);
				});
			return map;
		}
