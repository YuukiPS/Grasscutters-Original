...
public interface JsonAdapters {
	...
	class IntListAdapter extends TypeAdapter<IntList> {
		...
		public void write(JsonWriter writer, IntList l) throws IOException {
			writer.beginArray();
			for (val i : l) writer.value(i); // .forEach() doesn't appreciate exceptions
			writer.endArray();
		}
	}
	...
	class GridPositionAdapter extends TypeAdapter<GridPosition> {
		...
		public GridPosition read(JsonReader in) throws IOException {
			if (in.peek() != JsonToken.STRING) throw new IOException(
				"Invalid GridPosition definition - " + in.peek().name()
			);
			...
			if (split.length != 3) throw new IOException("Invalid GridPosition definition - " + in.peek().name());

			return new GridPosition(Integer.parseInt(split[0]), Integer.parseInt(split[1]), Integer.parseInt(split[2]));
		}
	}
