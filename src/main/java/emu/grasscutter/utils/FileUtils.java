...
public final class FileUtils {
		...
		try {
			...
			switch (uri.getScheme()) {
				...
				case "zip": // Honestly I have no idea what setup would result in this, but this should work
					// regardless
					fs = FileSystems.newFileSystem(uri, Map.of()); // Have to mount zip filesystem. This leaks, but we want to keep it
				// forever anyway.
				// Fall-through
	...
	public static List<Path> getPathsFromResource(String folder) throws URISyntaxException {
		try {
			// file walks JAR
			return Files
				.walk(Path.of(Grasscutter.class.getResource(folder).toURI()))
			...
			try {
				return Files
					.walk(Path.of(System.getProperty("user.dir"), folder))
					.filter(Files::isRegularFile)
					.collect(Collectors.toList());
