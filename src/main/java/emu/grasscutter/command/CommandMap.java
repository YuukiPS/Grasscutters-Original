...
public final class CommandMap {
	...
	private boolean setPlayerTarget(String playerId, Player player, String targetUid) {
		...
		CommandHandler.sendTranslatedMessage(
			player,
			targetPlayer.isOnline() ? "commands.execution.set_target_online" : "commands.execution.set_target_offline",
			target
		);
		return true;
	}
	...
	private void scan() {
		...
		classes.forEach(annotated -> {
			try {
				...
				if (object instanceof CommandHandler) this.registerCommand(
						cmdData.label(),
