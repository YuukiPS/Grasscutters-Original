						...
						default -> {
							if (DatabaseHelper.getPlayerByUid(Integer.parseInt(args.get(0))) != null) {
								...
								CommandHandler.sendMessage(
									sender,
									translate(sender, "commands.sendMail.user_not_exist", args.get(0))
								);
								return;
							}
						}
					...
					case "finish" -> {
						if (mailBuilder.constructionStage == 3) {
							if (!mailBuilder.sendToAll) {
								...
								DatabaseHelper
									.getByGameClass(Player.class)
									.forEach(player -> {
										var onlinePlayer = Grasscutter
											.getGameServer()
											.getPlayerByUid(player.getUid(), false);
					...
					default -> {
						switch (mailBuilder.constructionStage) {
							case 0 -> {
								...
								CommandHandler.sendMessage(
									sender,
									translate(sender, "commands.sendMail.set_title", title)
								);
							...
							case 1 -> {
								...
								CommandHandler.sendMessage(
									sender,
									translate(sender, "commands.sendMail.set_contents", contents)
								);
								mailBuilder.constructionStage++;
							}
							case 2 -> {
								...
								CommandHandler.sendMessage(
									sender,
									translate(sender, "commands.sendMail.set_message_sender", msgSender)
								);
								mailBuilder.constructionStage++;
							}
							case 3 -> {
								...
								switch (args.size()) {
									case 4: // <itemId|itemName> [amount] [level] [refinement] // TODO: this requires
										...
										try {
											...
											CommandHandler.sendMessage(
												sender,
												translate(sender, "commands.generic.invalid.itemRefinement")
											);
											return;
										} // Fallthrough
									case 3: // <itemId|itemName> [amount] [level]
										try {
											...
											CommandHandler.sendMessage(
												sender,
												translate(sender, "commands.generic.invalid.itemLevel")
											);
											return;
										} // Fallthrough
									case 2: // <itemId|itemName> [amount]
										try {
											...
											CommandHandler.sendMessage(
												sender,
												translate(sender, "commands.generic.invalid.amount")
											);
											return;
										} // Fallthrough
									case 1: // <itemId|itemName>
										try {
											...
											CommandHandler.sendMessage(
												sender,
												translate(sender, "commands.generic.invalid.itemId")
											);
											return;
										}
										break;
								...
								CommandHandler.sendMessage(
									sender,
									translate(sender, "commands.sendMail.send", amount, item, lvl)
								);
							}
						}
					}
