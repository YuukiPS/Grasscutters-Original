...
@Command(
	...
	aliases = { "taa" },
	usage = {
		"change <scheduleId>", "toggleDungeon <index(start from 1)|all>", "toggleReward <index(start from 1)|all>"
	},
...
public final class TrialAvatarActivityCommand implements CommandHandler {
	...
	public void execute(Player sender, Player targetPlayer, List<String> args) {
		...
		var playerDataOption = targetPlayer
			.getActivityManager()
		...
		var trialAvatarPlayerData = JsonUtils.decode(playerData.getDetail(), TrialAvatarPlayerData.class);
		...
		switch (action) {
			...
			case "change" -> {
				...
				if (TrialAvatarPlayerData.getAvatarIdList(Integer.parseInt(param)).isEmpty()) {
					CommandHandler.sendMessage(
						sender,
						translate(sender, "commands.trialAvatarActivity.schedule_not_found", Integer.parseInt(param))
					);
					return;
				}
				...
				CommandHandler.sendMessage(
					sender,
					translate(sender, "commands.trialAvatarActivity.success_schedule", Integer.parseInt(param))
				);
			}
			case "toggledungeon" -> {
				if (param.chars().allMatch(Character::isDigit)) { // if its number
					if (
						Integer.parseInt(param) - 1 >= trialAvatarPlayerData.getRewardInfoList().size() ||
						Integer.parseInt(param) - 1 <= 0
					) {
						CommandHandler.sendMessage(
							sender,
							translate(sender, "commands.trialAvatarActivity.invalid_param")
						);
						return;
					}
					TrialAvatarPlayerData.RewardInfoItem rewardInfo = trialAvatarPlayerData
						.getRewardInfoList()
						.get(Integer.parseInt(param) - 1);
					rewardInfo.setPassedDungeon(!rewardInfo.isPassedDungeon());
					playerData.setDetail(trialAvatarPlayerData);
					playerData.save();
					CommandHandler.sendMessage(
						sender,
						translate(sender, "commands.trialAvatarActivity.success_dungeon", Integer.parseInt(param))
					);
				} else {
					if (!param.equals("all")) {
						CommandHandler.sendMessage(
			...
			case "togglereward" -> {
				if (param.chars().allMatch(Character::isDigit)) { // if its number
					...
					CommandHandler.sendMessage(
						sender,
						translate(sender, "commands.trialAvatarActivity.success_reward", Integer.parseInt(param))
					);
				} else {
