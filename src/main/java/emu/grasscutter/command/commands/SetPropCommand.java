...
public final class SetPropCommand implements CommandHandler {
	...
	private boolean setTowerLevel(Player sender, Player targetPlayer, int topFloor) {
		...
		if (topFloor > 8) {
			recordMap.get(floorIds.get(7)).setLevelStars(0, 6); // levelIds seem to start at 1 for Floor 1 Chamber 1, so this doesn't get shown at
			// all
		}
		return true;
	}
	...
	private boolean unlockMap(Player targetPlayer) {
		...
		targetPlayer.sendPacket(
			new PacketScenePointUnlockNotify(playerScene, targetPlayer.getUnlockedScenePoints(playerScene))
		);
		targetPlayer.sendPacket(
			new PacketSceneAreaUnlockNotify(playerScene, targetPlayer.getUnlockedSceneAreas(playerScene))
		);
		return true;
	}
