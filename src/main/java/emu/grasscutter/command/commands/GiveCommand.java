...
public final class GiveCommand implements CommandHandler {
	...
	private static void giveAllAvatars(Player player, GiveItemParameters param) {
		...
		if (param.constellation < 0 || param.constellation > 6) param.constellation = 6; // constellation's default is -1 so if no parameters set for constellations it'll
		// automatically be 6
	...
	private static List<GameItem> makeArtifacts(GiveItemParameters param) {
		...
		for (int i = 0; i < param.amount; i++) {
			...
			if (
				param.mainPropId > 0
			) item.setMainPropId(param.mainPropId); // Keep random mainProp if we didn't specify one
	...
	private static int getArtifactMainProp(ItemData itemData, FightProperty prop) throws IllegalArgumentException {
		if (prop != FightProperty.FIGHT_PROP_NONE) for (ReliquaryMainPropData data : GameDepot.getRelicMainPropList(
			itemData.getMainPropDepotId()
	...
	private GiveItemParameters parseArgs(Player sender, List<String> args) throws IllegalArgumentException {
		...
		switch (id) {
			...
			default:
				...
				if ((param.id > 10_000_000) && (param.id < 12_000_000)) param.avatarData =
					GameData.getAvatarDataMap().get(param.id); else if (
					(param.id > 1000) && (param.id < 1100)
		...
		if (isRelic) {
			...
			param.lvl += 1;
			if (ILLEGAL_RELICS.contains(param.id)) CommandHandler.sendTranslatedMessage(
				sender,
				"commands.give.illegal_relic"
			);
		} else {
