...
public class SceneScriptManager {
	...
	public void resetTriggersForGroupSuite(SceneGroup group, int suiteIndex) {
		...
		if (suite == null) {
			Grasscutter
				.getLogger()
				.warn("Trying to load null suite Triggers for group {} with suiteindex {}", group.id, suiteIndex);
			return;
		}
	...
	public int refreshGroup(
		...
		if (
			waitForOne && (groupInstance.getTargetSuiteId() == 0 || prevSuiteIndex != groupInstance.getTargetSuiteId())
		) {
		...
		group.variables.forEach(variable -> {
			if (!variable.no_refresh) groupInstance.getCachedVariables().put(variable.name, variable.value);
		});
	...
	public boolean refreshGroupSuite(int groupId, int suiteId) {
		...
		if (targetGroupInstance == null) {
			getGroupById(groupId); // Load the group, this ensures an instance is created and the if neccesary
	...
	public boolean refreshGroupSuite(int groupId, int suiteId, GameQuest quest) {
		...
		if (suiteId != 0 && quest != null) {
			quest
				.getMainQuest()
				.getQuestGroupSuites()
				.add(QuestGroupSuite.of().scene(getScene().getId()).group(groupId).suite(suiteId).build());
		}
	...
	public boolean refreshGroupMonster(int groupId) {
		...
		var monstersToSpawn = group.monsters
			.values()
			.stream()
			.filter(m -> {
				...
				);/*&& !groupInstance.getDeadEntities().contains(entity); */// TODO: Investigate the usage of deadEntities
			})
	...
	public void registerRegion(EntityRegion region) {
		...
		Grasscutter
			.getLogger()
			.trace("Registered region {} from group {}", region.getMetaRegion().config_id, region.getGroupId());
	}
	...
	private static void addGridPositionToMap(
		Map<GridPosition, Set<Integer>> map,
		int group_id,
		int vision_level,
		Position position
	) {
	...
	private static int getGadgetVisionLevel(int gadget_id) {
		...
		for (int i = 0; i < visionOptions.length; i++) if (
			visionOptions[i].name.compareTo(gadget.getVisionLevel()) == 0
		) {
			return i;
		}
					...
					block.groups
						...
						.forEach(group -> {
							...
							if (group.regions != null) {
								group.regions
									.values()
									.forEach(r -> addGridPositionToMap(groupPositions.get(0), group.id, 0, r.pos));
							} else {
								Grasscutter.getLogger().error("group.regions null for group {}", group.id);
							}

							if (
								group.garbages != null && group.garbages.gadgets != null
							) group.garbages.gadgets.forEach(g ->
								addGridPositionToMap(
	...
	public void loadGroupFromScript(SceneGroup group) {
		...
		if (group.variables != null) {
			group.variables.forEach(variable -> {
				...
				if (variables != null && !variables.containsKey(variable.name)) variables.put(
					variable.name,
					variable.value
				);
			});
	...
	public void unregisterGroup(SceneGroup group) {
		...
		this.cachedSceneGroupsInstances.values()
			.stream()
	...
	public List<EntityGadget> getGadgetsInGroupSuite(SceneGroupInstance groupInstance, SceneSuite suite) {
		var group = groupInstance.getLuaGroup();
		return suite.sceneGadgets
			.stream()
			.filter(m -> {
				var entity = scene.getEntityByConfigId(m.config_id);
				return (
					(entity == null || entity.getGroupId() != group.id) &&
					(!m.isOneoff || !m.persistent || !groupInstance.getDeadEntities().contains(m.config_id))
				);
			})
	...
	public List<EntityMonster> getMonstersInGroupSuite(SceneGroupInstance groupInstance, SceneSuite suite) {
		...
		return suite.sceneMonsters
			...
			.filter(m -> {
				...
				return (
					entity == null || entity.getGroupId() != group.id
				);/*&& !groupInstance.getDeadEntities().contains(entity); */// TODO:
	...
	public void addGroupSuite(SceneGroupInstance groupInstance, SceneSuite suite, List<GameEntity> entities) {
		...
		if (entities != null) entities.addAll(toCreate); else this.addEntities(toCreate);
	...
	private void realCallEvent(@Nonnull ScriptArgs params) {
		try {
			...
			if (eventType == EventType.EVENT_ENTER_REGION || eventType == EventType.EVENT_LEAVE_REGION) {
				relevantTriggers =
					this.getTriggersByEvent(eventType)
						.stream()
						.filter(t ->
	...
	private boolean handleEventForTrigger(ScriptArgs params, SceneTrigger trigger) {
		Grasscutter.getLogger().trace("checking trigger {} for event {}", trigger.getName(), params.type);
		try {
			// setup execution
			...
			if (evaluateTriggerCondition(trigger, params)) {
				...
				Grasscutter.getLogger().trace("Condition Trigger {} returned false", trigger.getCondition());
			}
	...
	private void callTrigger(SceneTrigger trigger, ScriptArgs params) {
		...
		if (trigger.getEvent() == EventType.EVENT_ENTER_REGION) {
			var region = this.regions.values().stream().filter(p -> p.getConfigId() == params.param1).toList().get(0);
		...
		if (
			...
			ret.isint() &&
			ret.checkint() != 0 ||
			trigger.getTrigger_count() > 0 &&
			invocations >= trigger.getTrigger_count()
		) {
	...
	public EntityMonster createMonster(int groupId, int blockId, SceneMonster monster) {
		...
		this.getScriptMonsterSpawnService().onMonsterCreatedListener.forEach(action -> action.onNotify(entity));
	...
	public int createGroupTimerEvent(int groupID, String source, double time) {
		...
		if (group == null || group.triggers == null) {
			Grasscutter
				.getLogger()
				.debug("trying to create a timer for unknown group with id {} and source {}", groupID, source);
			return 1;
		}
		Grasscutter
			.getLogger()
			.debug("creating group timer event for group {} with source {} and time {}", groupID, source, time);
		for (SceneTrigger trigger : group.triggers.values()) {
			if (trigger.getEvent() == EVENT_TIMER_EVENT && trigger.getSource().equals(source)) {
				...
				var taskIdentifier = Grasscutter
					.getGameServer()
					.getScheduler()
					.scheduleDelayedRepeatingTask(
						() -> callEvent(new ScriptArgs(groupID, EVENT_TIMER_EVENT).setEventSource(source)),
						(int) time,
						(int) time
					);
	...
	public void onDestroy() {
		activeGroupTimers.forEach((gid, times) ->
			times.forEach(e -> Grasscutter.getGameServer().getScheduler().cancelTask(e.getSecond()))
		);
