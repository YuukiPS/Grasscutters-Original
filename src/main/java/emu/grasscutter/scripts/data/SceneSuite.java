...
public class SceneSuite {
	...
	public void init(SceneGroup sceneGroup) {
		...
		if (sceneGroup.gadgets != null) {
			this.sceneGadgets =
				new ArrayList<>(
					this.gadgets.stream().filter(sceneGroup.gadgets::containsKey).map(sceneGroup.gadgets::get).toList()
				);
		}
		...
		if (sceneGroup.regions != null) {
			this.sceneRegions =
				new ArrayList<>(
					this.regions.stream().filter(sceneGroup.regions::containsKey).map(sceneGroup.regions::get).toList()
				);
		}
	}
}
