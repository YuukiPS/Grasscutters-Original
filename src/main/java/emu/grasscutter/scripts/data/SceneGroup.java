...
public final class SceneGroup {
	...
	@Getter
	public boolean dynamic_load = false;
	...
	public synchronized SceneGroup load(int sceneId) {
		...
		var cs = ScriptLoader.getScript("Scene/%s/scene%s_group%s.lua".formatted(sceneId, sceneId, this.id));
		...
		try {
			...
			this.npcs =
				ScriptLoader
					.getSerializer()
					.toList(SceneNPC.class, this.bindings.get("npcs"))
					.stream()
					.collect(Collectors.toMap(x -> x.config_id, y -> y, (a, b) -> a));
			...
			this.init_config =
				ScriptLoader.getSerializer().toObject(SceneInitConfig.class, this.bindings.get("init_config"));
			...
			if (garbagesValue instanceof LuaValue garbagesTable) {
				...
				if (garbagesTable.checktable().get("gadgets") != LuaValue.NIL) {
					this.garbages.gadgets =
						ScriptLoader
							.getSerializer()
							.toList(SceneGadget.class, garbagesTable.checktable().get("gadgets").checktable());
			...
			Grasscutter
				.getLogger()
				.error("An error occurred while loading group " + this.id + " in scene " + sceneId + ".", e);
		} catch (LuaError luaError) {
			Grasscutter
				.getLogger()
				.error("An error occurred while loading group %s in scene %s.".formatted(this.id, sceneId), luaError);
		}
