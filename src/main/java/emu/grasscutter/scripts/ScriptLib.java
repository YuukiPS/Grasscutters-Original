...
public class ScriptLib {
	...
	public int AddExtraGroupSuite(int groupId, int suite) {
		...
		if (getSceneScriptManager().getScene().getChallenge() != null) if (
	...
	public int GetGroupMonsterCountByGroupId(int groupId) {
		...
		return (int) getSceneScriptManager()
			.getScene()
			.getEntities()
			.values()
			.stream()
	...
	public LuaValue ChangeGroupVariableValue(String var, int value) {
		...
		logger.debug("[LUA] Call ChangeGroupVariableValue with {},{}", old, old + value);
	...
	public int SetEntityServerGlobalValueByConfigId(int cfgId, String sgvName, int value) {
		...
		scene.broadcastPacket(new PacketServerGlobalValueChangeNotify(entity, sgvName, value));
		return 0;
	}
	...
	public int ChangeGroupVariableValueByGroup(String name, int value, int groupId) {
		logger.debug("[LUA] Call ChangeGroupVariableValueByGroup with {},{}", name, groupId);
		//TODO test
	...
	public int CheckRemainGadgetCountByGroupId(LuaTable table) {
		...
		var count = getSceneScriptManager()
			.getScene()
			.getEntities()
			.values()
			.stream()
	...
	public int MarkPlayerAction(int var1, int var2, int var3) {
		logger.debug("[LUA] Call MarkPlayerAction with {},{},{}", var1, var2, var3);
	...
	public int GadgetPlayUidOp(int groupId, int gadget_crucible, int var3, int var4, String var5, int var6) {
		logger.warn(
			"[LUA] Call unimplemented GadgetPlayUidOp with {}, {}, {}, {}, {}, {}",
			groupId,
			gadget_crucible,
			var3,
			var4,
	...
	public int IsPlayerAllAvatarDie(int sceneUid) {
		...
		var playerEntities = getSceneScriptManager()
			.getScene()
			.getEntities()
			.values()
			.stream()
			.filter(e -> e.getEntityType() == EntityIdType.AVATAR.getId())
			.toList();
	...
	public int ModifyFatherChallengeProperty(int challengeId, int propertyTypeIndex, int value) {
		...
		logger.warn(
			"[LUA] Call unimplemented ModifyFatherChallengeProperty with {} {} {}",
			challengeId,
			propertyType.name(),
			value
		);
	...
	public int AttachChildChallenge(int var1, int var2, int var3, int[] var4, LuaTable var5, LuaTable var6) {
		logger.warn(
			"[LUA] Call unimplemented AttachChildChallenge with {} {} {} {} {} {}",
			var1,
			var2,
			var3,
			var4,
	...
	public int SetHandballGalleryBallPosAndRot(int galleryId, LuaTable position, LuaTable rotation) {
		logger.warn(
			"[LUA] Call unimplemented SetHandballGalleryBallPosAndRot with {} {} {}",
			galleryId,
			printTable(position),
			printTable(rotation)
		);
	...
	public int MoveAvatarByPointArray(int uid, int targetId, LuaTable var3, String var4) {
		logger.warn(
			"[LUA] Call unimplemented MoveAvatarByPointArray with {} {} {} {}",
			uid,
			targetId,
			printTable(var3),
			var4
	...
	public int TransPlayerToPos(LuaTable var1) {
		...
		scene
			.getPlayers()
			.stream()
			.filter(p -> targets.contains(p.getUid()))
			.forEach(p -> {
				...
				p.sendPacket(
					new PacketPlayerEnterSceneNotify(
						p,
						EnterTypeOuterClass.EnterType.ENTER_TYPE_GOTO,
						Lua,
						scene.getId(),
						targetPos
					)
	...
	public int ScenePlaySound(LuaTable soundInfo) {
		...
		sceneScriptManager
			.get()
			.getScene()
			.broadcastPacket(new PacketScenePlayerSoundNotify(playPosition, soundName, playType));
		return 0;
	}
	...
	public int SetPlatformPointArray(int entityConfigId, int pointArrayId, int[] var3, LuaTable var4) {
		logger.warn(
			"[LUA] Call unimplemented SetPlatformPointArray with {} {} {} {}",
			entityConfigId,
			pointArrayId,
			var3,
			printTable(var4)
	...
	private int killGroupEntityWithPolicy(
		SceneScriptManager sceneScriptManager,
		SceneGroup group,
		GroupKillPolicy killPolicy
	) {
		// get targets
