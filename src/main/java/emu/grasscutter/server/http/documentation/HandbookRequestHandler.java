	...
	private List<String> generateHandbookHtmls(String template) {
		...
		CommandMap
			...
			.forEach(cmd -> {
				...
				for (int langIdx = 0; langIdx < NUM_LANGUAGES; langIdx++) sbs
					.get(langIdx)
					.append(
						"<tr><td><code>" +
						label +
		...
		avatarMap
			...
			.forEach(data -> {
				...
				for (int langIdx = 0; langIdx < NUM_LANGUAGES; langIdx++) sbs
					.get(langIdx)
					.append("<tr><td><code>" + id + "</code></td><td>" + name.get(langIdx) + "</td></tr>\n");
			});
		sbs.forEach(sb -> sb.setLength(sb.length() - 1)); // Remove trailing \n
		...
		sceneMap
			...
			.forEach(data -> {
				...
				for (int langIdx = 0; langIdx < NUM_LANGUAGES; langIdx++) sbs
					.get(langIdx)
					.append("<tr><td><code>" + id + "</code></td><td>" + data.getScriptData() + "</td></tr>\n");
			});
		sbs.forEach(sb -> sb.setLength(sb.length() - 1)); // Remove trailing \n
