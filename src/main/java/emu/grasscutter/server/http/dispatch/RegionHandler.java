...
public final class RegionHandler implements Router {
	...
	private void initialize() {
		...
		if (Grasscutter.getRunMode() != ServerRunMode.HYBRID && configuredRegions.size() == 0) {
			...
			System.exit(1);
		} else if (configuredRegions.size() == 0) configuredRegions.add(
			new Region(
				"os_usa",
		...
		configuredRegions.forEach(region -> {
			...
			var identifier = RegionSimpleInfo
				.newBuilder()
				.setName(region.Name)
		...
		var updatedRegionList = QueryRegionListHttpRsp
			.newBuilder()
		...
		// Create an updated region list.
		var updatedRegionListCN = QueryRegionListHttpRsp
			.newBuilder()
			.addAllRegionList(servers)
			.setClientSecretKey(ByteString.copyFrom(Crypto.DISPATCH_SEED))
			.setClientCustomConfigEncrypted(ByteString.copyFrom(encodedConfig))
	...
	private static void queryRegionList(Context ctx) {
		...
		if (ctx.queryParamMap().containsKey("version") && ctx.queryParamMap().containsKey("platform")) {
			...
			if (
				"CNRELiOS".equals(versionCode) || "CNRELWin".equals(versionCode) || "CNRELAndroid".equals(versionCode)
			) {
				...
				"OSRELiOS".equals(versionCode) || "OSRELWin".equals(versionCode) || "OSRELAndroid".equals(versionCode)
			) {
		...
		if (
			...
			try {
				...
				if (!clientVersion.equals(GameConstants.VERSION)) { // Reject clients when there is a version mismatch
					...
					QueryCurrRegionHttpRsp rsp = QueryCurrRegionHttpRsp
						.newBuilder()
	...
	public static QueryCurrRegionHttpRsp getCurrentRegion() {
		return Grasscutter.getRunMode() == ServerRunMode.HYBRID ? regions.get("os_usa").getRegionQuery() : null;
	}
}
