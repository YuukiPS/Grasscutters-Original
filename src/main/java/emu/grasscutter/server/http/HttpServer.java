...
public final class HttpServer {
	...
	public HttpServer() {
		...
		this.javalin =
			Javalin.create(config -> {
				...
				if (HTTP_ENCRYPTION.useEncryption) config.plugins.enableSslRedirects();
					...
					config.plugins.enableCors(cors ->
						cors.add(corsConfig -> {
							if (allowedOrigins.length > 0) {
								...
								) corsConfig.anyHost(); else corsConfig.allowHost(Arrays.toString(allowedOrigins));
				...
				if (DISPATCH_INFO.logRequests == ServerDebugMode.ALL) config.plugins.enableDevLogging();
		...
		this.javalin.exception(
				Exception.class,
				(exception, ctx) -> {
					ctx.status(500).result("Internal server error. %s".formatted(exception.getMessage()));
	...
	public void start() throws UnsupportedEncodingException {
		...
		Grasscutter
			.getLogger()
			.info(translate("messages.dispatch.address_bind", HTTP_INFO.accessAddress, this.javalin.port()));
	}
			...
			javalin.get(
				...
				ctx -> {
					...
					if (!file.exists()) {
						...
						ContentType fromExtension = ContentType.getContentTypeByExtension(
	...
	public static class UnhandledRequestRouter implements Router {
		...
		public void applyRoutes(Javalin javalin) {
			javalin.error(
				...
				ctx -> {
					// Error log
					if (DISPATCH_INFO.logRequests == ServerDebugMode.MISSING) Grasscutter
						.getLogger()
						.info(translate("messages.dispatch.unhandled_request_error", ctx.method(), ctx.url()));
					// Send file
