...
public interface IDispatcher {
	...
	Function<JsonElement, JsonObject> DEFAULT_PARSER = packet -> IDispatcher.decode(packet, JsonObject.class);
	 ...
	 */
	default <T> T await(JsonObject request, int requestId, int responseId, Function<JsonElement, T> parser) {
	...
	default void registerHandler(int packetId, BiConsumer<WebSocket, JsonElement> handler) {
		...
		if (this.getHandlers().containsKey(packetId)) throw new IllegalArgumentException(
			"Packet ID already registered."
		);
	...
	default void registerCallback(int packetId, Consumer<JsonElement> callback) {
		...
		if (!this.getCallbacks().containsKey(packetId)) this.getCallbacks().put(packetId, new LinkedList<>());
	...
	default void sendServerMessage(byte[] data, boolean binary) {
		var message = new JObject().add("binary", binary).add("data", Base64.getEncoder().encodeToString(data)).gson();
