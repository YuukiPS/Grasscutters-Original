...
public class PacketAllWidgetDataNotify extends BasePacket {
	...
	public PacketAllWidgetDataNotify(Player player) {
		...
		AllWidgetDataNotify.Builder proto = AllWidgetDataNotify
			.newBuilder()
		...
		if (player.getWidgetId() == 0) { // TODO: check this logic later, it was null-checking an int before which made it
			// dead code
			proto.addAllSlotList(List.of());
		} else {
			proto.addSlotList(
				WidgetSlotDataOuterClass.WidgetSlotData
					.newBuilder()
					.setIsActive(true)
