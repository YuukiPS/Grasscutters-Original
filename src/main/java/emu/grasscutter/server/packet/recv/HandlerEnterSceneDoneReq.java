...
public class HandlerEnterSceneDoneReq extends PacketHandler {
	...
	public void handle(GameSession session, byte[] header, byte[] payload) throws Exception {
		...
		var questGroupSuites = session
			.getPlayer()
			.getQuestManager()
			.getSceneGroupSuite(session.getPlayer().getSceneId());

		session.getPlayer().getScene().loadGroupForQuest(questGroupSuites);
		Grasscutter
			.getLogger()
			.trace("Loaded Scene {} Quest(s) Groupsuite(s): {}", session.getPlayer().getSceneId(), questGroupSuites);
