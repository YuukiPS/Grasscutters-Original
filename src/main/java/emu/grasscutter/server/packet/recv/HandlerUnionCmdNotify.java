...
public class HandlerUnionCmdNotify extends PacketHandler {
	...
	public void handle(GameSession session, byte[] header, byte[] payload) throws Exception {
		...
		for (UnionCmd cmd : req.getCmdListList()) {
			...
			if (
				GAME_INFO.logPackets == ServerDebugMode.WHITELIST && SERVER.debugWhitelist.contains(cmd.getMessageId())
			) {
				session.logPacket("RECV in Union", cmdOpcode, cmdPayload);
			} else if (
				GAME_INFO.logPackets == ServerDebugMode.BLACKLIST && !SERVER.debugBlacklist.contains(cmd.getMessageId())
			) {
				session.logPacket("RECV in Union", cmdOpcode, cmdPayload);
			}
