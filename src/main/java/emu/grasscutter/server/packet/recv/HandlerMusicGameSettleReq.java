...
public class HandlerMusicGameSettleReq extends PacketHandler {
	...
	public void handle(GameSession session, byte[] header, byte[] payload) throws Exception {
		...
		if (playerDataOpt.isEmpty()) {
			session.send(
				new PacketMusicGameSettleRsp(RetcodeOuterClass.Retcode.RET_MUSIC_GAME_LEVEL_CONFIG_NOT_FOUND, req)
			);
			return;
		}
		...
		if (req.getUgcGuid() == 0) {
			...
			isNewRecord =
				handler.setMusicGameRecord(
					playerData,
					MusicGamePlayerData.MusicGameRecord
						.of()
			...
			session.send(
				new PacketActivityInfoNotify(handler.toProto(playerData, activityManager.getConditionExecutor()))
			);
		} else {
			handler.setMusicGameCustomBeatmapRecord(
				playerData,
				MusicGamePlayerData.CustomBeatmapRecord
					.of()
