...
public interface Dumpers {
	...
	static void dumpScenes() {
		...
		GameData
			.getSceneDataMap()
			.forEach((id, scene) -> dump.put(id, new SceneInfo(scene.getScriptData(), scene.getSceneType())));
		...
		GameData
			...
			.forEach((id, monster) -> {
				...
				dump.put(
					id,
					new EntityInfo(
						langHash == 0 ? monster.getMonsterName() : Language.getTextMapKey(langHash).get(locale),
	...
	static void dumpQuests(String locale) {
		...
		GameData
			.getMainQuestDataMap()
			.forEach((id, mainQuest) -> {
				var langHash = mainQuest.getTitleTextMapHash();
				mainDump.put(
					id,
					new MainQuestInfo(
						langHash == 0 ? "Unknown" : Language.getTextMapKey(langHash).get(locale).replaceAll(",", "\\\\")
					)
				);
			});
