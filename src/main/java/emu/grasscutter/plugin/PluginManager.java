...
public final class PluginManager {
	...
	private void loadPlugin(Plugin plugin, PluginIdentifier identifier, URLClassLoader classLoader) {
		...
		try {
			...
			Method method = pluginClass.getDeclaredMethod(
				"initializePlugin",
	...
	public void registerListener(EventHandler<? extends Event> listener) {
		...
		if (!this.handlers.containsKey(listener.handles())) this.handlers.put(listener.handles(), new LinkedList<>());
	...
	private <T extends Event> void invokeHandler(Event event, EventHandler<T> handler) {
		if (!event.isCanceled() || (event.isCanceled() && handler.ignoresCanceled())) handler
			.getCallback()
			.consume((T) event);
	}
