...
public final class ResourceLoader {
	...
	public static void loadResources(boolean doReload) {
		...
		errors.forEach(pair ->
			Grasscutter.getLogger().error("Error loading resource file: " + pair.left(), pair.right())
		);
	...
	protected static void loadFromResource(Class<?> c, ResourceType type, Int2ObjectMap map, boolean doReload)
		throws Exception {
	...
	protected static <T> void loadFromResource(Class<T> c, String fileName, Int2ObjectMap map) throws Exception {
		JsonUtils
			.loadToList(getResourcePath("ExcelBinOutput/" + fileName), c)
			.forEach(o -> {
				GameResource res = (GameResource) o;
				res.onLoad();
				map.put(res.getId(), res);
	...
	private static void loadScenePoints() {
		...
		try {
			Files
				.newDirectoryStream(getResourcePath("BinOutput/Scene/Point"), "scene*_point.json")
				.forEach(path -> {
		...
		GameData
			...
			.forEach((id, data) ->
				GameData
					...
					.put((int) id, GameData.getProudSkillGroupLevels().get(data.getProudSkillGroupId()))
			);
		// Maximum known levels, keyed by proudSkillGroupId
		GameData
			.getProudSkillGroupLevels()
			.forEach((id, set) -> GameData.getProudSkillGroupMaxLevels().put((int) id, set.intStream().max().orElse(-1))
			);
	}
		...
		if (embryoList == null) {
			...
			try {
				GameDepot.setPlayerAbilities(
					JsonUtils.loadToMap(
						getResourcePath("BinOutput/AbilityGroup/AbilityGroup_Other_PlayerElementAbility.json"),
						String.class,
						AvatarConfig.class
					)
				);
	...
	private static void loadAbilityData(AbilityData data) {
		...
		modifiers.forEach((key, modifier) -> {
			...
			Stream
				.ofNullable(modifier.onThinkInterval)
				.flatMap(Stream::of)
				// .map(action -> {modifierActionTypes.add(action.$type); return action;})
				.filter(action -> action.type == AbilityModifierAction.Type.HealHP)
				.forEach(action -> modifierEntry.getOnThinkInterval().add(action));
			Stream
				.ofNullable(modifier.onRemoved)
				.flatMap(Stream::of)
				// .map(action -> {modifierActionTypes.add(action.$type); return action;})
				.filter(action -> action.type == AbilityModifierAction.Type.HealHP)
	...
	private static void loadTalent(Path path) {
		try {
			GameData
				.getTalents()
				.putAll(JsonUtils.loadToMap(path, String.class, new TypeToken<List<TalentData>>() {}.getType()));
		} catch (IOException e) {
			Grasscutter.getLogger().error("Error loading ability modifiers from path " + path.toString() + ": ", e);
		}
	}
	...
	private static void loadOpenConfig() {
		...
		if (list == null) {
			...
			for (String folderName : folderNames) {
				try {
					Files
						.newDirectoryStream(getResourcePath(folderName), "*.json")
						.forEach(path -> {
							try {
								JsonUtils
									.loadToMap(path, String.class, OpenConfigData[].class)
	...
	private static void loadQuests() {
		...
		try {
			...
			try {
				JsonUtils
					.loadToList(getResourcePath(path), QuestEncryptionKey.class)
					.forEach(key -> questEncryptionMap.put(key.getMainQuestId(), key));
			} catch (IOException | NullPointerException ignored) {}

			try {
				DataLoader
					.loadList(path, QuestEncryptionKey.class)
					.forEach(key -> questEncryptionMap.put(key.getMainQuestId(), key));
			} catch (IOException | NullPointerException ignored) {}
	...
	public static void loadScriptSceneData() {
		try {
			Files
				.list(getResourcePath("ScriptSceneData/"))
				.forEach(path -> {
					try {
						GameData
							.getScriptSceneDataMap()
							.put(path.getFileName().toString(), JsonUtils.loadToClass(path, ScriptSceneData.class));
					} catch (IOException e) {
						e.printStackTrace();
					}
				});
	...
	private static void loadHomeworldDefaultSaveData() {
		...
		try {
			Files
				.newDirectoryStream(getResourcePath("BinOutput/HomeworldDefaultSave"), "scene*_home_config.json")
				.forEach(path -> {
					val matcher = pattern.matcher(path.getFileName().toString());
					if (!matcher.find()) return;
			...
			Grasscutter
				.getLogger()
				.debug("Loaded " + GameData.getHomeworldDefaultSaveData().size() + " HomeworldDefaultSaveDatas.");
		} catch (IOException e) {
	...
	private static void loadNpcBornData() {
		try {
			Files
				.newDirectoryStream(getResourcePath("BinOutput/Scene/SceneNpcBorn/"), "*.json")
				.forEach(path -> {
					try {
						...
						data.setIndex(
							SceneIndexManager.buildIndex(3, data.getBornPosList(), item -> item.getPos().toPoint())
						);
	...
	private static void loadConfigLevelEntityData() {
		...
		try {
			var stream = Files.newDirectoryStream(
				getResourcePath("BinOutput/LevelEntity/"),
				"ConfigLevelEntity_*.json"
			);
			stream.forEach(path -> {
				val matcher = pattern.matcher(path.getFileName().toString());
				if (!matcher.find()) return;
	...
	private static void loadQuestShareConfig() {
		...
		try {
			...
			stream.forEach(path -> {
				...
				try {
					...
					var teleportDataMap = ScriptLoader
						.getSerializer()
						.toMap(TeleportData.class, bindings.get("quest_data"));
					var rewindDataMap = ScriptLoader
						.getSerializer()
					...
					GameData
						...
						.putAll(
							teleportDataMap
								.entrySet()
								.stream()
								.collect(Collectors.toMap(entry -> Integer.valueOf(entry.getKey()), Entry::getValue))
						);
					GameData
						.getRewindDataMap()
						.putAll(
					...
					Grasscutter
						.getLogger()
						.error("Error while loading Quest Share Config: {}", path.getFileName().toString());
				}
			});
			stream.close();
		} catch (IOException e) {
		...
		if (
			GameData.getTeleportDataMap() == null ||
	...
	private static void loadGadgetMappings() {
		try {
			...
			try {
				JsonUtils
					.loadToList(getResourcePath("Server/GadgetMapping.json"), GadgetMapping.class)
	...
	private static void loadMonsterMappings() {
		try {
			...
			try {
				JsonUtils
					.loadToList(getResourcePath("Server/MonsterMapping.json"), MonsterMapping.class)
	...
	private static void loadActivityCondGroups() {
		try {
			...
			try {
				JsonUtils
					.loadToList(getResourcePath("Server/ActivityCondGroups.json"), ActivityCondGroup.class)
		...
		try {
			...
			try {
				JsonUtils
					...
					.forEach(instance -> {
						instance.onLoad();
						GameData.getTrialAvatarActivityDataCustomData().put(instance.getTrialAvatarIndexId(), instance);
					});
			} catch (IOException | NullPointerException ignored) {}
			...
			try {
				JsonUtils
					...
					.forEach(instance -> {
						instance.onLoad();
						GameData.getTrialAvatarActivityCustomData().put(instance.getScheduleId(), instance);
					});
			} catch (IOException | NullPointerException ignored) {}
			Grasscutter.getLogger().debug("Loaded trial activity schedule custom data.");
			try {
				JsonUtils
					.loadToList(getResourcePath(pathName + "TrialAvatarData.json"), TrialAvatarCustomData.class)
					.forEach(instance -> {
						instance.onLoad();
	...
	private static void loadGroupReplacements() {
		...
		try {
			cs.eval(bindings);
			// these are Map<String, class>
			var replacementsMap = ScriptLoader
				.getSerializer()
			...
			GameData
				...
				.putAll(
					replacementsMap
						.entrySet()
						.stream()
						.collect(Collectors.toMap(entry -> Integer.valueOf(entry.getValue().getId()), Entry::getValue))
				);
		} catch (Throwable e) {
